...
protected:
	...
	// TSubclassOf는 지정한 클래스를 제한적으로 보여주는 필터링을 제공
	UPROPERTY(EditAnywhere, Category = Fight, meta = (AllowPrivateAccess = "true"))
	TSubclassOf<AABCharacterNonPlayer> OpponentClass;
...
protected:
	...
	TSubclassOf<AABItemBox> RewardItemClass;

	// 보상 상자는 스테이지와 무관하기 때문에 강참조(TObjectPtr)보다는 약참조(TWeakObjectPtr)를 사용하는 것이 좋음
	UPROPERTY(VisibleAnywhere, Category = Reward, meta = (AllowPrivateAccess = "true"))
	TArray<TWeakObjectPtr<AABItemBox>> RewardBoxes;
	...
	// Stage Stat
protected:
	// 스테이지 넘버를 스탯으로 관리 (NPC의 레벨로 활용 예정)
